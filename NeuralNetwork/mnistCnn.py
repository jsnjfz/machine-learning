...
if __name__ == '__main__':
    ...
    model.add(Conv2D(input_shape=(1, 28, 28), kernel_size=(5, 5), filters=4, padding="valid"))
    model.add(Activation('tanh'))

    # 第二个卷积层，8个卷积核，每个卷积核大小3*3。4表示输入的特征图个数，等于上一层的卷积核个数
    # model.add(Convolution2D(nb_filter=8, nb_row=3, nb_col=3, border_mode='valid'))
    model.add(Conv2D(kernel_size=(3, 3), filters=8, padding="valid"))
    model.add(Activation('tanh'))
    model.add(MaxPooling2D(pool_size=(2, 2)))

    # 第三个卷积层，16个卷积核，每个卷积核大小3*3
    # model.add(Convolution2D(nb_filter=16, nb_row=3, nb_col=3, border_mode='valid'))
    model.add(Conv2D(kernel_size=(3, 3), filters=16, padding="valid"))
    model.add(Activation('tanh'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    # 数据测试
    model.fit(data, label, batch_size=100, epochs=1, verbose=1, validation_split=0.2)
